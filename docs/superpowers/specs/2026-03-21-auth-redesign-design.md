...
**Логика оценки (из текущего AuthScreen):**
- < 8 символов: 0 точек, блокирует submit
- 8+ символов, только буквы: 1-2 точки (слабый)
- 8+ символов, буквы + цифры: 3 точки (средний)
- 8+ символов, буквы + цифры + спецсимволы: 4-5 точек (сильный)
- Минимум "средний" (3 точки) для submit
...
**Анимация:**
- GSAP timeline: анимация SVG path `d` через gsap.to с attr plugin (НЕ morphSVG — он платный)
...
- `forgot` — 1 карточка (email), затем переключается на `forgot-code` (6-digit), затем `forgot-reset` (новый пароль через PasswordCard)
