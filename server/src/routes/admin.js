...
// ═══ Все маршруты (кроме broadcast-update) требуют JWT + admin role ═══
// broadcast-update имеет fallback на X-Admin-Secret для deploy.py
...
// ─── GET /users ─── список юзеров с пагинацией ───
...
// ─── PATCH /users/:id ─── редактировать юзера ───
...
router.post('/users/:id/ban', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    const existing = await prisma.user.findUnique({ where: { id } });
    if (!existing) return res.status(404).json({ error: 'Пользователь не найден' });
    if (existing.role === 'admin') return res.status(400).json({ error: 'Нельзя забанить администратора' });
...
router.post('/users/:id/tags', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    console.error('admin/tag grant error:', err);
...
router.delete('/users/:id/tags/:tagId', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    console.error('admin/tag revoke error:', err);
...
router.patch('/tags/:id', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    console.error('admin/tags edit error:', err);
...
// ═══ broadcast-update (совместимость с deploy.py через X-Admin-Secret) ═══

router.post('/broadcast-update', async (req, res) => {
  // Двойная авторизация: JWT + admin role ИЛИ X-Admin-Secret (для deploy.py)
  const adminSecret = req.headers['x-admin-secret'];
  let isAuthed = false;

  // Проверка X-Admin-Secret (fallback для deploy.py)
  ...
  // Если нет секрета — проверить JWT
  if (!isAuthed) {
...
router.get('/server/config', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    console.error('admin/server/config error:', err);
